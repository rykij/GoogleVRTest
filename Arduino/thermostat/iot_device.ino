...
void loop()
{
  ...
  //to turn on/off light with button
  if(digitalRead(swLight) == 1)
  {
     digitalWrite(ledLight, !digitalRead(ledLight)); 
     Serial.println(digitalRead(swLight));
  }

  
  if(digitalRead(pir) == 1)
  {
    Serial.println("Presence detcted");
